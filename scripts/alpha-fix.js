...
function fixHexRgbaColorsInScss(file)
{
    const start = new Date();
    let replaceCount = 0;
    let fileContents = fs.readFileSync(file, { encoding:'utf8', flag:'r' });
    ...
    console.log('Replaced #xxxxxxxx to rgba(x,x,x,x) ' + replaceCount + ' times in ' + (new Date() - start) + 'ms in ' + file.split('/').slice(-1));
}

fixHexRgbaColorsInScss
